...
func TestEventLog_CreateArtifact_EmitsCreated(t *testing.T) {
	...
	if events[0].Scope != parchment.LabelValue(art.Labels, parchment.LabelPrefixScope) {
		t.Errorf("event scope = %q, want %q", events[0].Scope, parchment.LabelValue(art.Labels, parchment.LabelPrefixScope))
	}
}
