...
func TestSetField_UnknownField_Errors(t *testing.T) {
	...
	art, err := proto.CreateArtifact(ctx, parchment.CreateInput{
		Kind: parchment.KindTask, Title: "guinea pig", Scope: "test",
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetField_UnknownField_DoesNotPollutedExtra(t *testing.T) {
	...
	art, err := proto.CreateArtifact(ctx, parchment.CreateInput{
		Kind: parchment.KindTask, Title: "no pollution", Scope: "test",
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetField_KnownFields_StillWork(t *testing.T) {
	...
	cases := []struct {
		...
		{parchment.FieldPriority, "high", func(a *parchment.Artifact) bool { return a.Priority == "high" }},
		{parchment.FieldSprint, "s1", func(a *parchment.Artifact) bool { return a.Sprint == "s1" }},
	}
